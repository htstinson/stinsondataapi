...
func (d *Database) SelectSearchDefinitionEnginesView(ctx context.Context, subscriber model.Subscriber, limit, offset int) ([]model.SearchDefinitionEnginesView, error) {
	...
	query := fmt.Sprintf(`SELECT id, created_at, modified_at, search_engine_Id, search_definitions_id, 
	...
	for rows.Next() {
		...
		if err := rows.Scan(&row.Id, &row.CreatedAt, &row.ModifiedAt, &row.SearchEngineId, &row.SearchDefinitionsId,
