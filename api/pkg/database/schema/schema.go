...
func CopySchema(ctx context.Context, db *sql.DB, newSchemaName string) error {
	// Step 1: Create the new schema
	fmt.Printf("Creating schema: %s", newSchemaName)
	_, err := db.ExecContext(ctx, fmt.Sprintf("CREATE SCHEMA IF NOT EXISTS %s", newSchemaName))
	if err != nil {
	...
	seqRows, err := db.QueryContext(ctx, "SELECT sequence_name FROM information_schema.sequences WHERE sequence_schema = 'public'")
	if err != nil {
	...
	viewRows, err := db.QueryContext(ctx, "SELECT table_name FROM information_schema.views WHERE table_schema = 'public'")
	if err != nil {
	...
	for _, seqName := range sequences {
		fmt.Printf("Creating sequence: %s", seqName)

		// Get the current value of the sequence
		var lastVal int64
		var isCalled bool
		err := db.QueryRowContext(ctx, fmt.Sprintf("SELECT last_value, is_called FROM public.%s", seqName)).Scan(&lastVal, &isCalled)
		if err != nil {
			return fmt.Errorf("failed to get sequence value: %w", err)
		}

		// Create the sequence in the new schema
		_, err = tx.ExecContext(ctx, fmt.Sprintf("CREATE SEQUENCE %s.%s", newSchemaName, seqName))
		if err != nil {
		...
		if isCalled {
			_, err = tx.ExecContext(ctx, fmt.Sprintf("SELECT setval('%s.%s', %d, true)", newSchemaName, seqName, lastVal))
		} else {
			_, err = tx.ExecContext(ctx, fmt.Sprintf("SELECT setval('%s.%s', %d, false)", newSchemaName, seqName, lastVal))
		}
		if err != nil {
	...
	for _, tableName := range tables {
		fmt.Printf("Processing table: %s", tableName)
		...
		err := db.QueryRowContext(ctx, `
			...
			GROUP BY 
				c.relname;
		`, newSchemaName, tableName).Scan(&tableSQL)
	...
	for _, tableName := range tables {
		...
		`, newSchemaName, tableName, columnList, columnList, tableName))

		if err != nil {
			// Log error but continue with other tables
			fmt.Printf("Warning: Failed to copy data for table %s: %v", tableName, err)
		}
	}
	...
	for fkRows.Next() {
		...
		_, err = db.ExecContext(ctx, fkSQL)
		if err != nil {
			// Log error but continue with other constraints
			fmt.Printf("Warning: Failed to apply foreign key constraint: %v", err)
		}
	}
	...
	for idxRows.Next() {
		...
		if err != nil {
			...
			fmt.Printf("Warning: Failed to create index: %v", err)
		}
	}
	...
	for _, viewName := range views {
		...
		`, newSchemaName, viewName).Scan(&viewDef)
		...
		viewDef = strings.Replace(viewDef, "public.", newSchemaName+".", -1)

		// Create the view
		_, err = db.ExecContext(ctx, viewDef)
		if err != nil {
			fmt.Printf("Warning: Failed to create view %s: %v", viewName, err)
		}
	}

	fmt.Printf("Schema '%s' successfully created and populated", newSchemaName)
	return nil
}
